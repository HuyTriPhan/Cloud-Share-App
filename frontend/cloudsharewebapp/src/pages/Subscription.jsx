...
const Subscription = () => {
  ...
  useEffect(() => {
    ...
    if (payment === 'success') {
      const returnedCredits = params.get('credits');
  ...
  const handlePurchase = async (plan) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setMessage(response.data.message || "Failed to create payment order.");
        setMessageType("error");
      }
    } catch (error) {
  ...
  return (
    <DashboardLayout activeMeun="Subscription">
      <div className="p-6">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {plans.map((plan) => (
            ...
            >
              {plan.recommended && (
                <div className="inline-block bg-purple-500 text-white text-xs font-semibold px-2 py-1 rounded mb-2">
                  RECOMMENDED
                </div>
              )}
